//
// Created by cuongbv on 18/05/2020.
//
...
std::map<std::string, std::string> malwareHashMap;
...
std::string showUsage() {
    ...
    stringstream << "| Add new value: $ ./bf_client add <value> <malware>  |" << std::endl;
...
std::string loadInputFile(const std::string& inputFilePath) {
    ...
    std::vector<std::string> inputMalwareHashVector;
    ...
    while (!inputFile.eof()) {
        ...
        inputMalwareHashVector.push_back(hashValueString);
    }
    if (inputMalwareHashVector.empty()) {
    ...
    for (const std::string& malware_hash : inputMalwareHashVector) {
        std::vector<std::string> malware_hashContainer;
        splitString(malware_hash, malware_hashContainer, '\t');
        std::string malware(malware_hashContainer[0]);
        std::string hash(malware_hashContainer[1]);

        std::vector<int> hashIndex = hashFunc(hash);
        for (int i = 0; i < NUM_HASH_FUNCS; ++i) {
            bloomFilter[hashIndex[i]] = true;
        }
        malwareHashMap.insert(std::pair<std::string, std::string>(hash, malware));
        ++numElement;
    }
 ...
 *
 * @param inputHash: the value will be add into the Bloom filter.
 * @return A string, which is a notification about add input value procedure into the Bloom filter (at server side).
 */
std::string addValue(const std::string& inputHash, const std::string& malwareName) {
    std::stringstream stringstream;
    std::vector<int> hashIndex = hashFunc(inputHash);
    for (int i = 0; i < NUM_HASH_FUNCS; ++i) bloomFilter[hashIndex[i]] = true;
    malwareHashMap.insert(std::pair<std::string, std::string>(inputHash, malwareName));
    stringstream << "Complete add \"" << inputHash << "\" to the Bloom filter";
    startState = false;
    ++numElement;
    return stringstream.str();
}
...
std::string checkValue(const std::string& checkValue) {
    ...
    startState = false;
    if (isExist) stringstream << malwareHashMap[checkValue];
    else stringstream << "nonexistant";
    return stringstream.str();
}
